...
class Tpg extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $jenjangPengawas = $this->_helpLib->getJenjangPengawas($userId);

        $lists = $datamodel->get_datatables($jenjangPengawas, 'tpg');
        $data = [];
        ...
        foreach ($lists as $list) {
            ...
            //                 <a class="dropdown-item" href="javascript:actionSync(\'' . $list->id . '\', \'' . $list->id_pengawas . '\', \'' . str_replace("'", "", $list->nama)  . '\', \'' . $list->nuptk  . '\', \'' . $list->npsn . '\');"><i class="bx bx-transfer-alt font-size-16 align-middle"></i> &nbsp;Sync Dapodik</a>
            //             </div>
            //         </div>';
            $action = '<a href="javascript:actionDetail(\'' . $list->id . '\', \'' . $list->id_pengawas . '\', \'' . $list->id_tahun_tw . '\', \'' . str_replace('&#039;', "`", str_replace("'", "`", $list->nama)) . '\');"><button type="button" class="btn btn-primary btn-sm btn-rounded waves-effect waves-light mr-2 mb-1">
        ...
        $output = [
            "draw" => $request->getPost('draw'),
            "recordsTotal" => $datamodel->count_all($jenjangPengawas, 'tpg'),
            "recordsFiltered" => $datamodel->count_filtered($jenjangPengawas, 'tpg'),
            "data" => $data
        ];
        echo json_encode($output);
    }
    ...
    public function data()
    {
        ...
        return view('situpeng/peng/verifikasi/tpg/index', $data);
    }
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->data = view('situpeng/peng/verifikasi/tpg/detail', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function approve()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $oldData = $this->_db->table('_tb_temp_usulan_detail_pengawas')->where(['id' => $id])->get()->getRowObject();
            if (!$oldData) {
                $response = new \stdClass;
                $response->status = 201;
    ...
    public function formtolak()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->status = 200;
            $response->message = "Permintaan diizinkan";
            $response->data = view('situpeng/peng/verifikasi/tpg/tolak', $data);
            return json_encode($response);
        }
    }
    ...
    //                     return json_encode($response);
    //                 }
    //                 $response->data = view('situpeng/peng/us/ajukan/tpg-asn', $data);
    //             } else {
    ...
    //                     }
    //                 }
    //                 $response->data = view('situpeng/peng/us/ajukan/tpg-honor', $data);
    //             }
