...
use App\Models\Situpeng\Su\UserverifikasiModel;
use Config\Services;
...
class Grantedverifikasi extends BaseController
{
    ...
    {
        return redirect()->to(base_url('situpeng/su/setting/grantedverifikasi/data'));
    }
    ...
    public function data()
    {
        ...
        return view('situpeng/su/setting/grantedverifikasi/index', $data);
    }
    ...
    public function edit()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($val == '0') {
                $current = $this->_db->table('access_verifikasi_pengawas')
                    ->where('user_id', $id)->get()->getRowObject();

                if ($current) {
                    $this->_db->transBegin();
                    try {
                        $this->_db->table('access_verifikasi_pengawas')->where('id', $current->id)->delete();
                ...
                if (!$current) {
                    $this->_db->transBegin();
                    try {
                        $this->_db->table('access_verifikasi_pengawas')->insert(['user_id' => $id, 'created_at' => date('Y-m-d H:i:s')]);
