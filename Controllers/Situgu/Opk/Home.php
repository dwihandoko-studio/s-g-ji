...
class Home extends BaseController
{
    ...
    public function index()
    {
        ...
        $data['jumlah'] = $this->_db->table('_ptk_tb a')
            ->select("a.id, (SELECT count(id_ptk) FROM _ptk_tb WHERE npsn IN (select npsn from ref_sekolah where kode_kecamatan = '{$user->data->kecamatan}' AND bentuk_pendidikan_id = 5)) as jumlah_ptk, (SELECT count(id_ptk) FROM _ptk_tb WHERE id_kecamatan = a.id_kecamatan AND no_peserta IS NOT NULL) as jumlah_ptk_tpg, (SELECT count(id_ptk) FROM _ptk_tb WHERE id_kecamatan = a.id_kecamatan AND no_peserta IS NULL AND nuptk IS NOT NULL AND (status_kepegawaian IN ('PNS', 'PPPK', 'CPNS', 'PNS Depag', 'PNS Diperbantukan')) ) as jumlah_ptk_tamsil, (SELECT count(id_ptk) FROM _ptk_tb WHERE id_kecamatan = a.id_kecamatan AND no_peserta IS NULL AND nuptk IS NOT NULL AND (status_kepegawaian IN ('Guru Honor Sekolah', 'Honor Daerah TK.I Provinsi', 'Honor Daerah TK.II Kab/Kota','GTY/PTY')) ) as jumlah_ptk_pghm")
        ...
        return view('situgu/opk/home/index', $data);
    }
    ...
    public function getAktivasi()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($id == "admin") {
                ...
                $response->data = view('situgu/opk/home/aktivasi/admin', $x);
                return json_encode($response);
            } else if ($id == "email") {
                $x['user'] = $user->data;
                $response = new \stdClass;
                $response->status = 200;
                $response->message = "Permintaan diizinkan";
                $response->data = view('situgu/opk/home/aktivasi/email', $x);
            } else {
                $response = new \stdClass;
                $response->status = 400;
