...
namespace App\Controllers\Situgu\Opsr\Sptjm;

use App\Controllers\BaseController;
use App\Models\Situgu\Opsr\SptjmModel;
use Config\Services;
...
class Tamsil extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $lists = $datamodel->get_datatables($userId, 'tamsil');
        $data = [];
        ...
        foreach ($lists as $list) {
            ...
            if ($list->is_locked == 1) {
                ...
                if ($list->lampiran_sptjm == null || $list->lampiran_sptjm == "") {
                    $row[] = '<a class="btn btn-sm btn-primary waves-effect waves-light" target="_blank" href="' . base_url('situgu/opsr/sptjm/tamsil/download') . '?id=' . $list->kode_verifikasi . '"><i class="bx bxs-cloud-download font-size-16 align-middle me-2"></i> Download</a>&nbsp;&nbsp;'
        ...
        $output = [
            "draw" => $request->getPost('draw'),
            "recordsTotal" => $datamodel->count_all($userId, 'tamsil'),
            "recordsFiltered" => $datamodel->count_filtered($userId, 'tamsil'),
            "data" => $data
        ];
        echo json_encode($output);
    }
    ...
    {
        return redirect()->to(base_url('situgu/opsr/sptjm/tamsil/data'));
    }

    public function data()
    {
        $data['title'] = 'SPTJM VERIFIKASI TUNJANGAN TAMSIL';
        ...
        return view('situgu/opsr/sptjm/tamsil/index', $data);
    }
    ...
    public function add()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->data = view('situgu/opsr/sptjm/tamsil/add', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function generatesptjm()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if (!$twActive) {
                $response = new \stdClass;
                $response->status = 400;
                $response->message = "Gagal mengenerate SPTJM verifikasi tamsil. TW active tidak ditemukan.";
                return json_encode($response);
            }
            ...
            try {
                $kodeVerifikasi = "VTAMSIL-" . $twActive->tahun . '-' . $twActive->tw . '-' . time();
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    $response = new \stdClass;
                    $response->status = 200;
                    $response->message = "SPTJM Verifikasi Tamsil Tahun {$twActive->tahun} TW {$twActive->tw} berhasil digenerate.";
                    return json_encode($response);
                } else {
                    $this->_db->transRollback();
                    $response = new \stdClass;
                    $response->status = 400;
                    $response->message = "Gagal Mengenerate SPTJM Verifikasi Tamsil.";
                    return json_encode($response);
                }
            } catch (\Throwable $th) {
                $this->_db->transRollback();
    ...
    public function download()
    {
        ...
        if (!$twActive) {
            $response = new \stdClass;
            $response->status = 400;
            $response->message = "Gagal mendowload SPTJM verifikasi tamsil. TW active tidak ditemukan.";
            return json_encode($response);
        }
    ...
    private function _download($ptks, $user, $tw, $userDetail)
    {
        if (count($ptks) > 0) {
            ...
            $filed = FCPATH . "upload/generate/verifikasi/tamsil/word/" . $ptks[0]->kode_verifikasi . ".docx";
            ...
            $responseD =  $downloadLib->downloaded($filed, $ptks[0]->kode_verifikasi . ".pdf", "tamsil");
    ...
    public function formuploadedit()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->status = 200;
            $response->message = "Permintaan diizinkan";
            $response->data = view('situgu/opsr/sptjm/tamsil/upload_edit', $data);
            return json_encode($response);
        }
    }
    ...
    public function formupload()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->status = 200;
            $response->message = "Permintaan diizinkan";
            $response->data = view('situgu/opsr/sptjm/tamsil/upload', $data);
            return json_encode($response);
        }
    }
