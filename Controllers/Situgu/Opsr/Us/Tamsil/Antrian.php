...
use App\Models\Situgu\Opsr\Tamsil\AntrianModel;
use Config\Services;
...
class Antrian extends BaseController
{
    ...
    {
        return redirect()->to(base_url('situgu/opsr/us/tamsil/antrian/data'));
    }

    public function data()
    {
        $data['title'] = 'ANTRIAN USULAN TUNJANGAN TAMSIL';
        ...
        return view('situgu/opsr/us/tamsil/antrian/index', $data);
    }
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->data = view('situgu/opsr/verifikasi/tamsil/detail', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function formtolak()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->status = 200;
            $response->message = "Permintaan diizinkan";
            $response->data = view('situgu/opsr/verifikasi/tamsil/tolak', $data);
            return json_encode($response);
        }
    }
