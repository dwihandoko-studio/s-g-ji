...
namespace App\Controllers\Situgu\Opsr\Masterdata;

use App\Controllers\BaseController;
use App\Models\Situgu\Opsr\PtkModel;
use App\Models\Situgu\Opsr\SekolahModel;
use Config\Services;
...
class Ptk extends BaseController
{
    ...
    {
        return redirect()->to(base_url('situgu/opsr/masterdata/ptk/data'));
    }
    ...
    public function data()
    {
        ...
        return view('situgu/opsr/masterdata/ptk/index', $data);
    }
    ...
    public function sekolah()
    {
        ...
        $data['user'] = $user->data;
        $data['redirect'] = base_url('situgu/opsr/masterdata/ptk');
        $sekolah = $this->_db->table('ref_sekolah')->where('id', $id)->get()->getRowObject();
        if (!$sekolah) {
            return view('404', $data);
        }
        $data['sekolah'] = $sekolah;
        return view('situgu/opsr/masterdata/ptk/sekolah', $data);
    }
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->data = view('situgu/opsr/masterdata/ptk/detail', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function formhapus()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->status = 200;
            $response->message = "Permintaan diizinkan";
            $response->data = view('situgu/opsr/masterdata/ptk/hapus', $data);
            return json_encode($response);
        }
    }
