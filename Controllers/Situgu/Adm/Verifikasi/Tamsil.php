...
use App\Models\Situgu\Adm\VerifikasitamsilsekolahModel;
use App\Models\Situgu\Adm\VerifikasitamsildetailModel;
use Config\Services;
...
class Tamsil extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $lists = $datamodel->get_datatables('tamsil');
        $data = [];
    ...
    {
        return redirect()->to(base_url('situgu/adm/verifikasi/tamsil/data'));
    }
    ...
    public function data()
    {
        ...
        return view('situgu/adm/verifikasi/tamsil/index', $data);
    }
    ...
    public function datalist()
    {
        ...
        $data['kode_usulan'] = $id;
        $data['tw'] = $this->_db->table('_ref_tahun_tw')->where('is_current', 1)->orderBy('tahun', 'desc')->orderBy('tw', 'desc')->get()->getRowObject();
        return view('situgu/adm/verifikasi/tamsil/detail_index', $data);
    }
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->data = view('situgu/adm/verifikasi/tamsil/detail', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function formtolak()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->status = 200;
            $response->message = "Permintaan diizinkan";
            $response->data = view('situgu/adm/verifikasi/tamsil/tolak', $data);
            return json_encode($response);
        }
    }
    ...
    public function tolak()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $this->_db->transBegin();
            try {
                $this->_db->table('_tb_usulan_detail_tamsil')->where('id', $oldData->id)->update(['status_usulan' => 1, 'keterangan_reject' => $keterangan, 'date_reject' => date('Y-m-d H:i:s')]);
                if ($this->_db->affectedRows() > 0) {
                    $this->_db->transCommit();
                    $response = new \stdClass;
                    $response->status = 200;
