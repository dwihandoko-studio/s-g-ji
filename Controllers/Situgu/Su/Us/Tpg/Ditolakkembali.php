...
use App\Models\Situgu\Su\Tpg\DitolakModel;
use Config\Services;
...
class Ditolakkembali extends BaseController
{
    ...
    {
        return redirect()->to(base_url('situgu/su/us/tpg/ditolakkembali/data'));
    }

    public function data()
    {
        $data['title'] = 'USULAN DITOLAK TUNJANGAN PROFESI GURU';
        ...
        return view('situgu/su/us/tpg/ditolakkembali/index', $data);
    }
    ...
    public function kembalikan()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('v_ditolak_usulan_tpg a')
            ...
            if ($current) {
                ...
                try {
                    ...
                    if ($this->_db->affectedRows() > 0) {
                        $this->_db->table('_tb_usulan_detail_tpg')->where('id', $current->id_usulan)->update(['status_usulan' => 0, 'admin_reject' => NULL, 'date_reject' => NULL, 'keterangan_reject' => NULL]);
                        if ($this->_db->affectedRows() > 0) {
