...
use App\Models\Situgu\Su\VerifikasitamsilsekolahModel;
use App\Models\Situgu\Su\VerifikasitamsildetailModel;
use Config\Services;
...
class Tamsil extends BaseController
{
    ...
    {
        return redirect()->to(base_url('situgu/su/verifikasi/tamsil/data'));
    }
    ...
    public function data()
    {
        ...
        return view('situgu/su/verifikasi/tamsil/index', $data);
    }
    ...
    public function datalist()
    {
        ...
        $data['kode_usulan'] = $id;
        $data['tw'] = $this->_db->table('_ref_tahun_tw')->where('is_current', 1)->orderBy('tahun', 'desc')->orderBy('tw', 'desc')->get()->getRowObject();
        return view('situgu/su/verifikasi/tamsil/detail_index', $data);
    }
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->data = view('situgu/su/verifikasi/tamsil/detail', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
    ...
    public function formtolak()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->status = 200;
            $response->message = "Permintaan diizinkan";
            $response->data = view('situgu/su/verifikasi/tamsil/tolak', $data);
            return json_encode($response);
        }
    }
