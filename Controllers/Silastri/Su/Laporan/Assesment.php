...
namespace App\Controllers\Silastri\Su\Laporan;

use App\Controllers\BaseController;
use App\Models\Silastri\Su\Laporan\AssesmentModel;
use Config\Services;
...
class Assesment extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $datamodel = new AssesmentModel($request);
        ...
        foreach ($lists as $list) {
            ...
            //             </div>
            //         </div>';
            $action = '<a href="javascript:actionDetail(\'' . $list->id . '\', \'' . $list->nik_orang_assesment . '\', \'' . str_replace('&#039;', "`", str_replace("'", "`", $list->nama_orang_assesment)) . '\');"><button type="button" class="btn btn-primary btn-sm btn-rounded waves-effect waves-light mr-2 mb-1">
    ...
    {
        return redirect()->to(base_url('silastri/su/laporan/assesment/data'));
    }

    public function data()
    {
        $data['title'] = 'Data Laporan Assesment';
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
            session()->destroy();
        ...
        return view('silastri/su/laporan/assesment/index', $data);
    }
    ...
    public function detail()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                $response->data = view('silastri/su/laporan/assesment/detail', $data);
                return json_encode($response);
            } else {
                $response = new \stdClass;
                $response->status = 400;
