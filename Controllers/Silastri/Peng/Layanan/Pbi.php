...
class Pbi extends BaseController
{
    ...
    public function addSave()
    {
        ...
        if ($filenamelampiranPernyataan != '') {
            $lampiranValPernyataan = [
                '_file_sktm' => [
                    'rules' => 'uploaded[_file_sktm]|max_size[_file_sktm,2048]|mime_in[_file_sktm,image/jpeg,image/jpg,image/png,application/pdf]',
                    'errors' => [
        ...
        if (!$this->validate($rules)) {
            ...
            $kodeUsulan = "PBI-" . $user->data->nik . '-' . time();
