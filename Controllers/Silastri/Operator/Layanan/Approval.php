...
use App\Models\Silastri\Operator\Layanan\ApprovalModel;
use Config\Services;
...
class Approval extends BaseController
{
    ...
    {
        return redirect()->to(base_url('silastri/operator/layanan/approval/data'));
    }
    ...
    public function detail()
    {
        ...
        if ($current) {
            $data['data'] = $current;
            return view('silastri/operator/layanan/approval/detail-page', $data);
        } else {
    ...
    public function prosesttefromtemp()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $uploaded = $tteUpload->createUploadFile($dir_temp . $oldData['nik'] . '.pdf', $dir, $oldData['nik'] . '.pdf', $contentCreator, 'http://192.168.33.16:8020/generate?data=https://layanan.dinsos.lampungtengahkab.go.id/verifiqrcode?token=' . $oldData['id']);
