...
use App\Models\Silastri\Adm\Pengaduan\SelesaiModel;
use Config\Services;
...
class Selesai extends BaseController
{
    ...
    {
        return redirect()->to(base_url('silastri/adm/pengaduan/selesai/data'));
    }

    public function data()
    {
        $data['title'] = 'Pengaduan Layanan Selesai';
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
            session()->destroy();
        ...
        return view('silastri/adm/pengaduan/selesai/index', $data);
    }
        ...
        if (!$this->validate($rules)) {
            ...
            if ($current) {
                ...
                if ($granted) {
                    ...
                    $response->data = view('silastri/adm/pengaduan/selesai/detail', $data);
                    return json_encode($response);
                } else {
                    $response = new \stdClass;
                    $response->status = 400;
