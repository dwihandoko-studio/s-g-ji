        ...
        fetch("./get_data_json?id=<?= $id ?>")
            ...
            .then(data => {
                ...
                for (let i = 0; i < data.data.length; i++) {
                    ...
                    const skPangkatCell = document.createElement("td");
                    const tmtPangkatCell = document.createElement("td");
                    const mkTahunCell = document.createElement("td");
                    const mkBulanCell = document.createElement("td");
                    const skPangkatUploadCell = document.createElement("td");
                    const tmtPangkatUploadCell = document.createElement("td");
                    const mkTahunUploadCell = document.createElement("td");
                    const mkBulanUploadCell = document.createElement("td");
                    const aksiCell = document.createElement("td");
                    numberCell.textContent = 1 + i;
                    nuptkCell.textContent = data.data[i].nuptk;
                    namaCell.textContent = data.data[i].nama;
                    skPangkatCell.textContent = data.data[i].sk_pengangkatan;
                    tmtPangkatCell.textContent = data.data[i].tmt_pangkat;
                    mkTahunCell.textContent = data.data[i].mk_tahun;
                    mkBulanCell.textContent = data.data[i].mk_bulan;
                    skPangkatUploadCell.textContent = data.data[i].sk_pengangkatan_upload;
                    tmtPangkatUploadCell.textContent = data.data[i].tmt_pangkat_upload;
                    mkTahunUploadCell.textContent = data.data[i].mk_tahun_upload;
                    mkBulanUploadCell.textContent = data.data[i].mk_bulan_upload;
                    aksiCell.textContent = data.data[i].aksi;
                    row.appendChild(numberCell);
                    row.appendChild(nuptkCell);
                    row.appendChild(namaCell);
                    row.appendChild(skPangkatCell);
                    row.appendChild(tmtPangkatCell);
                    row.appendChild(mkTahunCell);
                    row.appendChild(mkBulanCell);
                    row.appendChild(skPangkatUploadCell);
                    row.appendChild(tmtPangkatUploadCell);
                    row.appendChild(mkTahunUploadCell);
                    row.appendChild(mkBulanUploadCell);
            ...
            let sendToServer = function(lines, index) {
                if (index > lines.length - 1) {
                    ...
                    Swal.fire(
                        ...
                        document.location.href = "<?= base_url('silastri/su/test'); ?>";
                    })
