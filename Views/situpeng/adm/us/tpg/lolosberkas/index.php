...
<script>
    function actionDownload(event) {
        ...
        console.log(tw);
        if (tw === undefined || tw === "") {
            const linkSource = "<?= base_url('situpeng/adm/us/tpg/lolosberkas/download') ?>?tw=<?= $tw->id ?>";
            ...
            const linkSource = "<?= base_url('situpeng/adm/us/tpg/lolosberkas/download') ?>?tw=" + tw;
            const downloadLink = document.createElement("a");
            downloadLink.href = linkSource;
            downloadLink.onclick = function() {
                window.open(this.href);
    ...
    function actionEdit(id, id_pengawas, tw, nama, nuptk) {
        $.ajax({
            url: "./edit",
            type: 'POST',
            data: {
            ...
            success: function(resul) {
                ...
                if (resul.status !== 200) {
                    ...
                    $('#content-detailModalLabel').html('EDIT USULAN PENGAWAS ' + nama);
    ...
    function actionDetail(id, id_pengawas, tw, nama, nuptk) {
        $.ajax({
            url: "./detail",
            type: 'POST',
            data: {
