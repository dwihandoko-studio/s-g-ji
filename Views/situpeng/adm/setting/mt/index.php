        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0 font-size-18">SETTING MAINTENANCE</h4>

                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <?php if ($status_mt > 0) { ?>
                                <li class="breadcrumb-item"><a href="javascript:actionDisabled(this);" class="btn btn-danger btn-rounded waves-effect waves-light">Non Aktifkan Maintenance</a></li>
                            <?php } else { ?>
                                <li class="breadcrumb-item"><a href="javascript:actionActived(this);" class="btn btn-success btn-rounded waves-effect waves-light">Aktifkan Maintenance</a></li>
                            <?php } ?>
                            <li class="breadcrumb-item"><a href="javascript:actionAdd(this);" class="btn btn-primary btn-rounded waves-effect waves-light">Add Template MT</a></li>
                        </ol>
                    </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-6">
                                <h4 class="card-title">Data Template Maintenace</h4>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <table id="data-datatables" class="table table-bordered dt-responsive  nowrap w-100">
                            <thead>
                                <tr>
                                    ...
                                    <th>NAMA</th>
                                    <th>
                                        <div class="text-center">AKTIF TEMPLATE</div>
                                    </th>
                                </tr>
                            </thead>
                        </table>
...
<script>
    function actionDisabled(event) {
        Swal.fire({
            title: 'Apakah anda yakin ingin menonaktifkan mode maintenance?',
            text: "Nonaktifkan Mode Maintenance : situpeng",
            ...
            if (result.value) {
                $.ajax({
                    url: "<?= base_url('situpeng/su/setting/mt/disabled') ?>",
                    type: 'POST',
                    data: {
                        id: 'disable',
                    },
    ...
    function actionActived(event) {
        Swal.fire({
            title: 'Apakah anda yakin ingin mengaktifkan mode maintenance?',
            text: "Aktifkan Mode Maintenance : situpeng",
            showCancelButton: true,
            icon: 'question',
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            if (result.value) {
                $.ajax({
                    url: "<?= base_url('situpeng/su/setting/mt/active') ?>",
                    type: 'POST',
                    data: {
                        id: 'active',
                    },
    ...
    function actionDetail(event, title) {
        $.ajax({
            url: "<?= base_url('situpeng/su/masterdata/sekolah/detail') ?>",
            type: 'POST',
            data: {
                id: event,
            },
    ...
    $(document).ready(function() {
        ...
        let tableDatatables = $('#data-datatables').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?= base_url('situpeng/su/setting/mt/getAll') ?>",
                "type": "POST",
